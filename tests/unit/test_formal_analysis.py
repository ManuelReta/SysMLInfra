...
import pytest
import examples.bilgepump.formal_analysis as bilgepump_fa

# Skip entire module if z3 is not available
z3 = pytest.importorskip(
    "z3", reason="z3-solver not installed; skipping formal analysis tests"
)
...
def test_level_1_proves_req001_at_nominal():
    n = bilgepump_fa._nominal()
    result = bilgepump_fa.level_1_symbolic_baseline(n, verbose=False)
...
def test_level_1_returns_formal_result():
    n = bilgepump_fa._nominal()
    result = bilgepump_fa.level_1_symbolic_baseline(n, verbose=False)
    assert isinstance(result, bilgepump_fa.FormalResult)
...
def test_level_2_finds_gap_at_nominal():
    ...
    """
    n = bilgepump_fa._nominal()
    result = bilgepump_fa.level_2_efficiency_floor(n, verbose=False)
...
def test_level_5_finds_gap_with_original_reqs():
    ...
    """
    n = bilgepump_fa._nominal()
    result = bilgepump_fa.level_5_adversarial(n, verbose=False)
...
def test_run_all_level_subset():
    results = bilgepump_fa.run_all(levels=[1, 2])
