...
def _parse_connections(arch_path: str) -> list[tuple[str, str, str]]:
    ...
    clean = _strip_comments(raw)
    conns = []
    for m in re.finditer(r"\bconnect\s+([\w.]+)\s+to\s+([\w.]+)\s*;", clean):
...
def _diagram_system_topology(
    ...
    nx.draw_networkx_nodes(
        G, pos, ax=ax, node_color=node_colors, node_size=2200, alpha=0.92
    )
    ...
    edge_labels = {
        (s, t): d["label"] for s, t, d in G.edges(data=True) if d.get("label")
    }
    nx.draw_networkx_edge_labels(
        G, pos, edge_labels=edge_labels, ax=ax, font_size=6.5, label_pos=0.35
    )
    ...
    legend_entries = [
        mpatches.Patch(
            color=role_colors.get(inst, "#BDBDBD"),
    ...
    ax.set_title(
        "SysML v2 System — Architecture Topology", fontsize=13, fontweight="bold"
    )
    ax.axis("off")
    fig.tight_layout()
...
def _diagram_requirement_status(
    ...
    req_nodes = [
        n for n, d in G.nodes(data=True) if d.get("node_type") == "requirement"
    ]
    ...
    nx.draw_networkx_nodes(
        ...
        nodelist=req_nodes,
        ax=ax,
        node_color=req_colors,
        node_size=2800,
        node_shape="s",
    )
    nx.draw_networkx_nodes(
        G, pos, nodelist=comp_nodes, ax=ax, node_color=comp_colors, node_size=1800
    )
...
def _diagram_traceability(
    ...
    for fm in fmea_modes:
        ...
        if fid:
            rpn = fm.get("rpn", "")
            G.add_node(
                f"FM:{fid}", node_type="fm", color="#EF9A9A", label=f"{fid}\nRPN={rpn}"
            )
    ...
    ax.set_title(
        "Safety Traceability — Requirements ↔ UCAs ↔ FMEA",
        fontsize=13,
        fontweight="bold",
    )
    ax.axis("off")
...
def generate_all(
    ...
    if arch_path and os.path.exists(arch_path):
        ...
        try:
            ...
            print(
                f"  ✓ system_topology.png  ({len(parts)} parts, {len(connections)} connections)"
            )
    ...
    if req_subjects and (ucas or fmea_modes):
        ...
        try:
            ...
            print(
                f"  ✓ traceability.png  ({len(ucas)} UCAs, {len(fmea_modes)} FMEA modes)"
            )
        except Exception as exc:
