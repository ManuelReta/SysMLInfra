...
def build_bind_index(layer_paths: list[str], repo_root: str, negative: bool = False) -> dict:
    """
...
def build_uca_index(safety_path: str, repo_root: str) -> dict:
    ...
    for block_m in re.finditer(r'#UCA\s*\{([^}]+)\}', clean, re.DOTALL):
        ...
        for field in ('ucaId', 'controlAction', 'guideword', 'context',
                      'hazardRefs', 'severity', 'failureModeLink', 'transitionRef',
                      'sourceDoc', 'section'):
...
def build_fmea_index(fmea_path: str, repo_root: str) -> dict:
    ...
    for block_m in re.finditer(r'#FailureMode\s*\{([^}]+)\}', clean, re.DOTALL):
        ...
        # String fields
        for field in ('fmId', 'component', 'instance', 'failureModeText',
                      'failureEffect', 'ucaRef', 'hazardRef', 'stateRef',
                      'sourceDoc', 'section'):
            fm = re.search(rf'{field}\s*=\s*"([^"]*)"', block_text)
            if fm:
                entry[field] = fm.group(1)
...
def _find_ucas_for_component(
    ...
    for comp in component_names:
        ...
        for uid, uca in uca_index.items():
            ...
            short = re.sub(r'[A-Z]', lambda x: x.group().lower(), comp)  # camelCase → lower
            if (comp_lower in ca or comp_lower in fm_link
                    or short in ca or short in fm_link
...
class FaultTrace:
    ...
    def format(self, color: bool = True) -> str:
        ...
        reg_str = f"[{', '.join(self.reg_ids)}]" if self.reg_ids else ''
        lines   = [
        ...
        if self.bind_hits:
            ...
            for b in self.bind_hits:
                marker = '  ◀ FAULT' if b['value'] == 0.0 and 'flow' in b['path'].lower() else ''
                lines.append(f"    {b['path']} = {b['value']}  [{b['file']}:{b['line']}]{marker}")
        else:
            lines.append(f"  {YELLOW}Bind values{RESET} : (no direct bind found — check Analysis.sysml)")
        ...
        if self.ucas:
            ...
            lines.append(f"  {YELLOW}UCA trace{RESET}   : (no UCA linked to this component)")
        ...
        if self.fmea_modes:
            ...
            for fm in self.fmea_modes:
                ...
                lines.append(
                    f"      Effect: {fm.get('failureEffect','?')}"
                )
                lines.append(f"      [{fm.get('file','?')}:{fm.get('line','?')}]")
        else:
            lines.append(f"  {YELLOW}FMEA trace{RESET}  : (no FMEA failure mode linked)")
...
class FaultTracer:
    ...
    def __init__(self, repo_root: str, all_layer_paths: list[str], negative: bool = False):
    ...
    def load(self) -> None:
        ...
        self._bind_index = build_bind_index(self.layer_paths, self.repo_root, self.negative)
