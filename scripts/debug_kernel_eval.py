...
import verify  # noqa: E402  (after path manipulation)
...
def _read_manifest_layers() -> list[str]:
    _, all_layers, validation_layers = verify._read_manifest(MANIFEST)
...
def main() -> None:
    ...
    print(f'  Mode   : {yellow("negative (pump A failure)" if args.negative else "positive")}')
    ...
    eval_cells = verify._build_kernel_eval_cells(layer_paths, args.negative)

    _banner('Substituted expressions (spaces stripped for single-token %eval)')
    print(f'  {dim("Spaces removed so each %eval receives exactly ONE positional token.")}')
    print(f'  {dim("Kernel wraps as: calc { <compact_expr>; } → ExpressionEvaluator.INSTANCE")}')
    print()
    ...
    for lp in layer_paths:
        nb.cells.append(nbformat.v4.new_code_cell(verify._read(os.path.join(REPO_ROOT, lp))))
    ...
    _banner(f'Executing kernel — loading {n_model} layer(s) + {len(eval_cells)} %eval cell(s)')
    print(f'  {dim("(SysML kernel startup takes ~10 s — kernel boot messages suppressed)")}')
    sys.stdout.flush()
    sys.stderr.flush()
    _so = os.dup(1); _se = os.dup(2)
    ...
    try:
        ...
        os.dup2(_so, 1); os.dup2(_se, 2)
        os.close(_so);   os.close(_se)
    ...
    _banner('Step 2 — Requirement evaluation via kernel %eval → ExpressionEvaluator.INSTANCE')
    print(f'  {dim("Each cell: %eval <compact_expr>  (no spaces = single token)")}')
    print(f'  {dim("Kernel: calc { <compact_expr>; } → ExpressionEvaluator.INSTANCE.evaluate()")}')
    print()
    ...
    for j, (req_name, expr, label) in enumerate(eval_cells):
        ...
        if not outputs:
            print(f'  Kernel raw  : {yellow("(cell never executed — check prior errors)")}')
    ...
    if n_unknown:
        print(f'  {yellow("UNKNOWN")}   : {n_unknown}  (expr not substituted, kernel error, or not executed)')
    else:
