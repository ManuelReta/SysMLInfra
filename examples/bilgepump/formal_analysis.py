...
def level_2_efficiency_floor(n: dict, verbose: bool) -> FormalResult:
    ...
    eta_nominal = n["effA"]
    _ = (flow_a + flow_b) - q_req  # req004_margin
    ft002_nominal = eta_nominal * total_flow * loss_factor
    ft002_margin = ft002_nominal - q_req

    _ = eta_nominal < eta_min_analytic  # should be False at η=0.82  gap_exists
    ...
    if check == z3.sat:
        ...
        return FormalResult(
            ...
            counterexample={
                "eta": cex_eta,
                "net_flow": cex_eta * total_flow * loss_factor,
            },
        )
    elif check == z3.unsat:
        return FormalResult(
...
def level_4_response_window(n: dict, verbose: bool) -> FormalResult:
    ...
    if check == z3.sat:
        ...
        detail_parts.append(
            "G-4: REQ-005 (response ≤ 5s) + REQ-003 (alarm ≤ 2s) do not prevent this."
        )
    else:
        detail_parts.append(
            "At nominal timing and inflow, FT-004 cannot be violated (UNSAT)."
        )
    ...
    outcome = (
        "GAP"
        if (check == z3.sat or check2 == z3.sat)
        else ("PROVED" if check == z3.unsat else "UNKNOWN")
    )
    cex = {}
    if check2 == z3.sat:
...
def level_5_adversarial(n: dict, verbose: bool) -> FormalResult:
    ...
    if check == z3.sat:
        ...
        net = (cex["flowA"] * cex["effA"] + cex["flowB"] * cex["effB"]) * (
            1.0 - cex["pipeLoss"]
        )
...
def _print_results(results: list[FormalResult], verbose: bool, color: bool) -> None:
    ...
    print("─" * W)
    print(
        f"  Results: {len(proved)} PROVED  {len(gaps)} GAP(s)  {len(unknown)} UNKNOWN"
    )
    if gaps:
...
def main() -> None:
    ...
    parser.add_argument(
        "--level", type=int, nargs="+", help="Run specific levels (1–6). Default: all."
    )
    parser.add_argument(
        "--verbose", action="store_true", help="Show Z3 counterexample variable values."
    )
    parser.add_argument(
    ...
    _ = sum(1 for r in results if r.outcome == "GAP")  # gaps
